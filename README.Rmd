...
```
`DFOsamplestats` also provides functions to estimate the effect size ad p-values 
...
As a result it is quicker but less reliable at small sample sizes than `rate2_power_analysis()` which performs a full power analysis although calculation of the p-values assumes that the likelihood profile is normally distributed.

To understand why consider the fact that it is not possible to get a significant
